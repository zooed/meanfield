...
class EpisodesBuffer(Buffer):
    ...
    def episodes(self):
        """ get episodes """
        return self.buffer.values() # train()中调用 获取字典的值
...
class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, models,
        ...
        env: magent.GridWorld
            environment handle交互的环境由magent定义
        handles: list
            group handles
        map_size: int
            map size of grid world网格世界的图大小
        max_steps: int
            the maximum of stages in a episode 一局游戏中的最大的步数
        render_every: int
        ...
        self.play = play_handle # play函数的引用
        self.model_dir = model_dir
        self.train = train # 默认为True
        ...
        if self.train:
            ...
            # self_play 操作 ???
            self.sp_op = [tf.assign(r_vars[i], (1. - tau) * l_vars[i] + tau * r_vars[i]) for i in range(len(l_vars))]
